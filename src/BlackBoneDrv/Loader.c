...
NTSTATUS BBInitLdrData( IN PKLDR_DATA_TABLE_ENTRY pThisModule )
{
    ...
    if (kernelBase == NULL)
    {
        DPRINT( "Failed to retrieve Kernel base address. Aborting\n" );
    ...
    {
        DPRINT( "Failed to retrieve PsLoadedModuleList address. Aborting\n" );
        return STATUS_NOT_FOUND;
    }
...
PVOID BBGetUserModule( IN PEPROCESS pProcess, IN PUNICODE_STRING ModuleName, IN BOOLEAN isWow64 )
{
    ...
    __try
    {
        ...
        if (isWow64)
        {
            ...
            if (pPeb32 == NULL)
            {
                DPRINT( "No PEB present. Aborting\n" );
                return NULL;
            }
            ...
            {
                DPRINT( "Loader not intialiezd, waiting\n" );
            ...
            if (!pPeb32->Ldr)
            {
                DPRINT( "Loader was not intialiezd in time. Aborting\n" );
                return NULL;
            }
...
NTSTATUS BBLookupProcessThread( IN PEPROCESS pProcess, OUT PETHREAD* ppThread )
{
    ...
    if (!pInfo)
    {
        DPRINT( "Failed to allocate memory for process list\n" );
    ...
    if (NT_SUCCESS( status ))
    {
        ...
        DPRINT( "Failed to locate process\n" );
    ...
    )
{
    ...
    if (NT_SUCCESS( status ) && wait != FALSE)
    {
        ...
        if (NT_SUCCESS( status ))
        {
            ...
            {
                DPRINT( "ZwQueryInformationThread failed with status 0x%X\n", status );
            }
        }
        else
            DPRINT( "ZwWaitForSingleObject failed with status 0x%X\n", status );
    }
    else
        DPRINT( "ZwCreateThreadEx failed with status 0x%X\n", status );
    ...
    )
{
    ...
    //DPRINT( "Called. NormalRoutine = 0x%p\n", *NormalRoutine );
...
NTSTATUS BBMapWorker( IN PVOID pArg )
{
    ...
    if (NT_SUCCESS( status ))
    {
        ...
        {
            DPRINT( "Failed to allocate memory for image '%wZ'\n", pPath );
    ...
    if (NT_SUCCESS( status ))
        DPRINT( "Successfully mapped '%wZ' at 0x%p\n", pPath, imageSection );
...
NTSTATUS BBMMapDriver( IN PUNICODE_STRING pPath )
{
    ...
    if (!NT_SUCCESS( status ))
    {
        DPRINT( "Failed to create worker thread. Status: 0x%X\n", status );
        return status;
    }
