...
NTSTATUS BBBuildProcessRegionListForRange( IN PLIST_ENTRY pList, IN ULONG_PTR start, IN ULONG_PTR end, IN BOOLEAN mapSections )
{
    ...
    for (memptr = start; memptr < end; memptr = (ULONG_PTR)mbi.BaseAddress + mbi.RegionSize)
    {
        ...
        {
            DPRINT( "ZwQueryVirtualMemory for address 0x%p returned status 0x%X\n", memptr, status );
        ...
        else if (mbi.State == MEM_COMMIT &&  mbi.Protect != PAGE_NOACCESS && !(mbi.Protect & PAGE_GUARD))
        {
            ...
            if (pEntry == NULL)
            {
                DPRINT( "Failed to allocate memory for Remap descriptor\n" );
...
NTSTATUS BBUnmapRegionEntry( IN PMAP_ENTRY pPageEntry, IN PPROCESS_MAP_ENTRY pFoundEntry )
{
    ...
    if (pPageEntry->pMdl)
    {
        ...
        {
            DPRINT( "Unmapping region at 0x%p from process %u\n", pPageEntry->newPtr, pFoundEntry->target.pid );
...
NTSTATUS BBHandleSharedRegion( IN PEPROCESS pProcess, IN PMAP_ENTRY pEntry )
{
         ...
         memptr = (ULONG_PTR)mbi.BaseAddress + mbi.RegionSize)
    {
        ...
        if (!NT_SUCCESS( status ))
        {
            DPRINT( "ZwQueryVirtualMemory for address 0x%p failed, status 0x%X\n", memptr, status );
            return status;
        }
        ...
        if (writable || NT_SUCCESS( status = ZwProtectVirtualMemory( ZwCurrentProcess(), &pBase, &size, PAGE_EXECUTE_READWRITE, &oldProt ) ))
        {
            ...
            {
                DPRINT( "Exception while touching address 0x%p\n", mbi.BaseAddress );
                failed = TRUE;
            }
            ...
            for (ULONG_PTR pPage = (ULONG_PTR)mbi.BaseAddress; pPage < (ULONG_PTR)mbi.BaseAddress + mbi.RegionSize; pPage += PAGE_SIZE)
            {
                ...
                {
                    DPRINT( "Page at address 0x%p is still shared!", pPage );
                    return STATUS_SHARING_VIOLATION;
                }
            }
        }
        else
        {
            if(status != STATUS_SECTION_PROTECTION)
                DPRINT( "Failed to alter protection of region at 0x%p, status 0x%X\n", mbi.BaseAddress, status );
...
NTSTATUS BBPrepareMDLListEntry( IN PEPROCESS pProcess, IN OUT PMAP_ENTRY pEntry )
{
    ...
    if (pEntry->pMdl == NULL)
    {
        DPRINT( "Failed to allocate MDL for address 0x%p\n", pEntry->mem.BaseAddress );
        return STATUS_NO_MEMORY;
    }
    ...
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
        DPRINT( "Exception in MmProbeAndLockPages. Base = 0x%p\n", pEntry->mem.BaseAddress );
...
NTSTATUS BBMapRegionIntoCurrentProcess( IN PMAP_ENTRY pEntry, IN PMAP_ENTRY pPrevEntry )
{
    ...
    if (pEntry->newPtr != 0)
    {
        //if (pEntry->newPtr != pEntry->originalPtr)
            //DPRINT( "Remapped 0x%p --> 0x%p\n", pEntry->originalPtr, pEntry->newPtr );
    ...
    {
        DPRINT( "Failed to map region at adress 0x%p\n", pEntry->mem.BaseAddress );
...
NTSTATUS BBMapRegionListIntoCurrentProcess( IN PLIST_ENTRY pList, IN BOOLEAN noWarning )
{
    ...
    for (PLIST_ENTRY pListEntry = pList->Flink; pListEntry != pList; pListEntry = pListEntry->Flink)
    {
        ...
        {
            if (noWarning == FALSE)
                DPRINT( "Warning! Region 0x%p already mapped to 0x%p\n", pEntry->mem.BaseAddress, pEntry->newPtr );
        ...
        if (pEntry->pMdl && pEntry->locked)
            ...
            DPRINT( "No valid MDL for address 0x%p. Either not allocated or not locked\n", pEntry->mem.BaseAddress );
...
NTSTATUS BBAllocateSharedPage( OUT PVOID* pPage, OUT PMDL* pResultMDL )
{
    ...
    *pPage = ExAllocatePoolWithTag( NonPagedPool, PAGE_SIZE, BB_POOL_TAG );
    ...
    {
        DPRINT( "Failed to allocate kernel page\n" );
...
NTSTATUS BBMapSharedPage( IN PMDL pMDL, OUT PVOID* pResult )
{
    ...
    if (*pResult == NULL)
    {
        DPRINT( "Failed to map kernel page\n" );
...
NTSTATUS BBMapMemory( IN PMAP_MEMORY pRemap, OUT PPROCESS_MAP_ENTRY* ppEntry )
{
    ...
    if (!NT_SUCCESS( status ))
    {
        DPRINT( "Failed to find process with PID %u\n", processEntry.target.pid );
    ...
    {
        DPRINT( "Process %u is terminating. Abort\n", processEntry.target.pid );
    ...
    if (newEntry != FALSE)
    {
        ...
        if (NT_SUCCESS( status ))
        {
            ...
            if (NT_SUCCESS( status ) && pRemap->pipeName[0] != L'\0')
            {
                ...
                if (!NT_SUCCESS( pipeStatus ))
                    DPRINT( "Remap pipe failed with status 0x%X\n", pipeStatus );
            }
    ...
    {
        DPRINT( "Found mapping entry for process %u\n", pFoundEntry->target.pid );
        ...
        {
            DPRINT( "Host process %u still exists. Cannot map into new process\n", pFoundEntry->host.pid );
...
NTSTATUS BBMapMemoryRegion( IN PMAP_MEMORY_REGION pRegion, OUT PMAP_MEMORY_REGION_RESULT pResult )
{
    ...
    {
        DPRINT( "Invalid address range: 0x%p - 0x%p\n", pRegion->base, pRegion->base + pRegion->size );
    ...
    if (!NT_SUCCESS( status ))
    {
        DPRINT( "Failed to find process 0x%u\n", processEntry.target.pid );
        KeReleaseGuardedMutex( &g_globalLock );
        return status;
    }
    ...
    if (newEntry != FALSE)
    {
        DPRINT( "Process entry 0x%u was not found, creating new\n", processEntry.target.pid );
        InitializeListHead( &pFoundEntry->pageList );
    ...
    else
    {
        ...
        if (pFoundEntry->host.pid != PsGetCurrentProcessId() && pFoundEntry->host.pid != NULL)
        {
            DPRINT( "Call from non-original process not supported\n" );
        ...
        if (pPageEntry)
        {
            ...
            {
                DPRINT(
                    "Target region 0%p - 0x%p conficts with existing: 0x%p - 0x%p\n",
                    pRegion->base, pRegion->base + pRegion->size,
...
NTSTATUS BBUnmapMemory( IN PUNMAP_MEMORY pUnmap )
{
    ...
    if (pFoundEntry == NULL)
    {
        DPRINT( "Process entry with PID %u was not found\n", pUnmap->pid );
        status = STATUS_NOT_FOUND;
    }
    else
    {
        DPRINT( "BBUnmapMemory cleanup for PID %u\n", pUnmap->pid );
        BBCleanupProcessEntry( pFoundEntry );
    }
...
NTSTATUS BBUnmapMemoryRegion( IN PUNMAP_MEMORY_REGION pRegion )
{
    ...
    if (pFoundEntry == NULL)
    {
        DPRINT( "Process entry for process %u was not found\n", pRegion->pid );
        status = STATUS_NOT_FOUND;
    }
    else
    {
        ...
        else
        {
            ...
            {
                DPRINT( "Map entry for address 0x%p was not found\n", pRegion->base );
...
VOID BBCleanupHostProcess( IN PPROCESS_MAP_ENTRY pProcessEntry )
{
    ...
    DPRINT( "Host process %u shutdown. Cleanup\n", pProcessEntry->host.pid );
